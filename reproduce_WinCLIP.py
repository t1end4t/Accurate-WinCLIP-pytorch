...
class prompt_order:
    ...
    def prompt(self, class_name):
        ...
        normal_ensemble_template = [
            class_template.format(ele)
            for ele in class_state
            for class_template in self.template_list
        ]

        class_state = [
            ele.format(class_name) for ele in self.state_anomaly_list
        ]
        anomaly_ensemble_template = [
            class_template.format(ele)
            for ele in class_state
            for class_template in self.template_list
        ]
...
class patch_scale:
    ...
    def make_mask(self, patch_size=16, kernel_size=16, stride_size=16):
        ...
        self.idx_board = torch.arange(
            1, self.patch_num_h * self.patch_num_w + 1, dtype=torch.float32
        ).reshape((1, 1, self.patch_num_h, self.patch_num_w))
        patchfy = torch.nn.functional.unfold(
            self.idx_board,
...
class CLIP_AD(nn.Module):
    def __init__(self, model_name="ViT-B-16-plus-240"):
        ...
        self.model, _, self.preprocess = (
            open_clip.create_customer_model_and_transforms(
    ...
    def encode_image(self, image, patch_size, mask=True):
        if mask:
            b, _, _, _ = image.shape
            large_scale = (
                self.mask.make_mask(kernel_size=48, patch_size=patch_size)
                .squeeze()
                .cuda()
            )
            mid_scale = (
                self.mask.make_mask(kernel_size=32, patch_size=patch_size)
                .squeeze()
                .cuda()
            )
            tokens_list, class_tokens, patch_tokens = self.model.encode_image(
                image, [large_scale, mid_scale], proj=False
            )
            large_scale_tokens, mid_scale_tokens = (
                tokens_list[0],
                tokens_list[1],
            )
            return (
...
def compute_score(image_features, text_features):
    ...
    text_probs = (
        torch.bmm(image_features.unsqueeze(1), text_features) / 0.07
    ).softmax(dim=-1)
...
def compute_sim(image_features, text_features):
    ...
    text_features /= text_features.norm(dim=1, keepdim=True)
    simmarity = (
        torch.bmm(image_features.squeeze(2), text_features) / 0.07
    ).softmax(dim=-1)
    return simmarity
...
def harmonic_aggregation(score_size, simmarity, mask):
    ...
    for idx in range(h * w):
        patch_idx = [
            bool(torch.isin(idx + 1, mask_patch)) for mask_patch in mask
        ]
        ...
        for c in range(C):
            for i in range(H):
                for j in range(W):
                    ...
                    harmonic_sum = 0
                    for u in range(
                        i * stride - overlap, i * stride + overlap + 1
                    ):
                        for v in range(
                            j * stride - overlap, j * stride + overlap + 1
                        ):
...
def few_shot(memory, token, class_name):
    ...
    M = (
        ...
        * torch.min(
            1.0
            - torch.bmm(
                F.normalize(token.squeeze(2), dim=-1),
                F.normalize(retrive, dim=1),
            ),
            dim=-1,
        )[0]
    )
...
def prepare_text_future(model, obj_list):
    ...
    for i in obj_list:
        ...
        abnormal_text_features = model.encode_text(
            abnormal_tokens.cuda()
        ).float()

        avg_normal_text_features = torch.mean(
            normal_text_features, dim=0, keepdim=True
        )
        avg_abnormal_text_features = torch.mean(
            abnormal_text_features, dim=0, keepdim=True
        )
    ...
    Mermory_avg_normal_text_features = torch.stack(
        Mermory_avg_normal_text_features
    )
    Mermory_avg_abnormal_text_features = torch.stack(
        Mermory_avg_abnormal_text_features
    )
...
def test(
    ...
    Mermory_avg_normal_text_features, Mermory_avg_abnormal_text_features = (
        prepare_text_future(model, obj_list)
    )
    ########################################
    if args.k_shot == 0:
        few = False
    ...
    if few:
        large_memory, mid_memory, patch_memory = memory(
            model.to(device),
            obj_list,
            dataset_dir,
            save_path,
    ...
    for index, items in enumerate(tqdm(test_dataloader)):
        ...
        (
            ...
            large_scale,
            mid_scale,
        ) = model.encode_image(images, patch_size)
        ...
        if few:
            ...
            m_l = harmonic_aggregation(
                (b, h // patch_size, w // patch_size), m_l, large_scale
            ).cuda()
            m_m = harmonic_aggregation(
                (b, h // patch_size, w // patch_size), m_m, mid_scale
            ).cuda()
            m_p = m_p.reshape((b, h // patch_size, w // patch_size)).cuda()

            few_shot_score = torch.nan_to_num(
                (m_l + m_m + m_p) / 3.0, nan=0.0, posinf=0.0, neginf=0.0
            )

        zscore = compute_score(
            class_tokens,
            torch.cat(
                (average_normal_features, average_anomaly_features), dim=1
            ).permute(0, 2, 1),
        )

        z0score = zscore[:, 0, 1]

        large_scale_simmarity = compute_sim(
            large_scale_tokens,
            torch.cat(
                (average_normal_features, average_anomaly_features), dim=1
            ).permute(0, 2, 1),
        )[:, :, 1]
        mid_scale_simmarity = compute_sim(
            mid_scale_tokens,
            torch.cat(
                (average_normal_features, average_anomaly_features), dim=1
            ).permute(0, 2, 1),
        ...
        #####################################multi-scale
        large_scale_score = harmonic_aggregation(
            (b, h // patch_size, w // patch_size),
            large_scale_simmarity,
            large_scale,
        )
        mid_scale_score = harmonic_aggregation(
            (b, h // patch_size, w // patch_size),
            mid_scale_simmarity,
            mid_scale,
        )
        ...
        multiscale_score = torch.nan_to_num(
            3.0
            / (
                1.0 / large_scale_score.cuda()
                + 1.0 / mid_scale_score.cuda()
                + 1.0 / z0score.unsqueeze(1).unsqueeze(1)
            ),
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        ...
        multiscale_score = multiscale_score.cuda().unsqueeze(
            1
        )  # Add batch and channel dimensions

        if few:
            multiscale_score = (
                multiscale_score + few_shot_score.cuda().unsqueeze(1)
            )

            z0score = (
                z0score
                + torch.max(torch.max(few_shot_score, dim=1)[0], dim=1)[0]
            ) / 2.0

        multiscale_score = F.interpolate(
            multiscale_score, size=(h, w), mode="bilinear"
        )
    ...
    results["anomaly_maps"] = (
        torch.cat(results["anomaly_maps"]).detach().cpu().numpy()
    )
...
if __name__ == "__main__":
    ...
    # model
    parser.add_argument(
        "--dataset", type=str, default="mvtec", help="test dataset"
    )
    parser.add_argument(
        "--model", type=str, default="ViT-B-16", help="model used"
    )
    parser.add_argument(
        "--pretrained",
        type=str,
    ...
    # few shot
    parser.add_argument(
        "--k_shot", type=int, default=10, help="10-shot, 5-shot, 1-shot"
    )
